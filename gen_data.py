...
def download_data():
    ...
    for name, url in datasets.items():
        if not os.path.exists(f'./datos/{name}.csv'):
            with open(f'./datos/{name}.csv')
                archivo.write( request.get(url).content.decode() )

def clean_dp1():
    DataProduct1 = pd.read_csv("./datos/dp1_contagios.csv", parse=["Fecha"])
    # Just RM
...
def clean_df82():
    ...
    return val_salidas, paso_a_paso
